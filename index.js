...
function addGamesToPage(games) {
    ...
    // loop over each item in the data
        games.forEach(game => {
            ...
            // about each game
            gameDiv.innerHTML = `
                <img src=${game.img}  alt="Cover image of ${game.name}" class="game-img">
                <h2>${game.name}</h2>
...
const displayStr = 
    `A total of ${totalPledged.toLocaleString()} has been raise for ${GAMES_JSON.length} ${(GAMES_JSON.length > 1) ? "games" : "game"}. Currently, ${numUnfunded} ${(numUnfunded > 1) ? "games remain" : "game remains"} unfunded. We need your help to fund these amazing games!`

// create a new DOM element containing the template string and append it to the description container
const callToAction = document.createElement("p");
callToAction.innerHTML = displayStr;
